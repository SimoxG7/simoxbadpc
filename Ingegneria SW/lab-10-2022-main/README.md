...
* TEAMMATE 1: Rivera Simone 922755
* TEAMMATE 2: Gioè Simone 931106
...
* Per quanto riguarda i `Presenter`: l'approccio usato nel diagramma di sequenza prevede una classe unica (`TemperatureModel`). Le due strategie `CelsiusScale` e `FahrenheitScale` possono essere realizzate usando il pattern  `Singleton`.
 ...
 * le classi di test per `TemperatureModel`, per `TemperaturePresenter` (nel source set `test`) e per il sistema una volta completato (nel source set `integrationTest`).
