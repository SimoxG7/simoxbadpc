...
def main():
    ...
    while cmd != "q\n":
        ...
        if cmd == "h": #working
            ...
            for cocktail in templist:
                names.append(cocktail.get_name())
                simoxrate.append(cocktail.get_simoxrate())
                ingredcount.append(len(cocktail.get_ingredients()))


            #fig, ax = plt.subplots()
            plt.rc("figure", figsize=(100, 100))
            plt.bar(names, simoxrate)
            plt.xticks(size=6, rotation=90)
            #fig.fmt_xdate()
            plt.ylabel("SimoxRate")
            plt.xlabel("Cocktails")
            plt.show()

            plt.rc("figure", figsize=(100, 100))
            plt.bar(names, ingredcount)
            plt.xticks(size=6, rotation=90)
            #fig.fmt_xdate()
