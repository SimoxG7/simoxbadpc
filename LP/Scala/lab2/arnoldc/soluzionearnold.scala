...
class ArnoldCCombinators(var the_stack: Stack[Int], var the_table: HashMap[String,Int]) extends JavaTokenParsers {

  def arnoldc_program = "IT'S" ~> "SHOW" ~> "TIME" ~> arnoldc_body <~ "YOU" <~ "HAVE" <~ "BEEN" <~ "TERMINATED" ^^ {_ => (the_stack,the_table)}
  ...
  def var_def = "HEY" ~> "CHRISTMAS" ~> "TREE" ~> ident ~("YOU" ~> "SET" ~> "US" ~> "UP"~> intvalue) ^^ { case s~n => the_table(s) = n; }
  ...
  def ifthenelse = "BECAUSE" ~> "I'M" ~> "GOING" ~> "TO" ~> "SAY" ~> "PLEASE" ~ idvalue ~ block ~ ("BULLSHIT" ~> block <~ "YOU" <~ "HAVE" <~ "NO" <~ "RESPECT" <~ "FOR" <~ "LOGIC") ^^ {
    case e~b1~b2 => {
  ...
  def expr = (arithmetic|logic) ^^ { (f:(Int => Int)) => the_stack.push(f(the_stack.pop)) }
  ...
  def assignment = "GET" ~> "TO" ~>"THE" ~> "CHOPPER" ~> ident <~ ("HERE" ~> "IS" ~> "MY" ~> "INVITATION" ~> initial_value) <~ exprs <~ "ENOUGH" <~ "TALK" ^^ {
