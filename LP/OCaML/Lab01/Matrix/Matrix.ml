...
struct
  ...
  let identity n = 
    List.init n (fun x -> List.init n (fun y -> if (x <> y) then 0 else 1))
  ;;
  ...
  let vect_prod v1 v2 = 
    let rec aux res v1 v2 = match v1,v2 with 
      | h1::t1, h2::t2 -> aux (res + (h1*h2)) t1 t2 (* pensa di dover usare il + tra matrici!*)
      | _, _ -> res 
    in aux 0 v1 v2
  ;;

  let ( + ) m1 m2 = 
    List.map2 (fun r1 r2 -> List.map2 (fun v1 v2 -> v1+v2) r1 r2) m1 m2 (*mappa due liste*)
  ;;
